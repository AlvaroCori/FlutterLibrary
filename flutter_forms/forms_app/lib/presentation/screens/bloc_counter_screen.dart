...
class BlockCounterView extends StatelessWidget {
  ...
  void increaseCounterBy(BuildContext context, [ int value = 1]){
    //context.read<CounterBloc>().add( CounterIncrease(value: value ));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: context.select((CounterBloc bloc) => Text("Bloc counter ${ bloc.state.counter}")),
        actions: [
      ...
      body: Center(
        child: context.select((CounterBloc counterBloc) =>
            Text("here ${counterBloc.state.counter}")),
      ),
